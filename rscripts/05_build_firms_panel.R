...
# Parameter to choose firm size
#  / large / all
firm_size <- "all" # 
if(firm_size == "small"){
  cnbv_reports <- c("R443","R464")
}else if(firm_size == "large"){
  cnbv_reports <- c("R469","R484")
} else if(firm_size == "all"){
  cnbv_reports <- c("R443","R464","R469","R484")
}
...
# load TCR
tcr <- read.csv(paste(path, "/tcr/tcr_2015_2022.csv",
                       sep = ""))
tcr <- tcr %>% 
  mutate(Fecha = Fecha %>% lubridate::dmy() )
...
# triming very large firms, 0.01% percentile, 2023-07-08
n_orig_4 <- nrow(data_r_all)
cutoff <-  quantile(data_r_all$loan_disposition_month,.99)
data_r_all <- with(data_r_all, 
                   data_r_all[clean( !(loan_disposition_month > cutoff), 
                                     include = TRUE),])

n_clean4 <- nrow(data_r_all)
n_removed_int4 <- n_orig_4 - n_clean4; n_removed_int4
100* (n_orig_3 - n_clean3) / n_orig 

# cleaning only group of interest
data_r_all <- data_r_all %>% 
  mutate(firm_size = 
           case_when(cnbv_report_code  %in%  c("R443","R464") ~ "S",
                     cnbv_report_code  %in%  c("R469","R484") ~ "L") )
             
n_orig_4 <- nrow(data_r_all)
data_r_all <- data_r_all[data_r_all$cnbv_report_code %in% cnbv_reports,]
n_clean3 <- nrow(data_r_all)
...
# IV a la Hausmann -----------------------
# via Felipe BruguEs 
periods <- unique(data_r_all$period)
sectors <- unique(cat_sects$scian_code[order(cat_sects$scian_code)])


#temp <- data_r_all %>% filter(period == "2016-06-01", 
#                              institution_code == "040002")

#temp2 <- data_r_all %>% filter(period == "2016-06-01")

get_wa_interest_rates_mkt <- function(data, var2){
  # data <- temp; var2 <- "11"

  i_sector <- data %>% group_by(scian_code) %>% 
    filter(scian_code == var2) %>% 
    summarise(i_sector = sum(interest_rate_disposition * loan_disposition_month, na.rm = TRUE) / sum(loan_disposition_month, na.rm = TRUE)) %>% 
    pull(i_sector)
  if(length(i_sector) == 0 )
    i_sector <- NA
  
  i_other_sector <- data %>% filter(scian_code != var2) %>% 
    summarise(i_other_sector = sum(interest_rate_disposition * loan_disposition_month, na.rm = TRUE) / sum(loan_disposition_month, na.rm = TRUE)) %>% 
    pull(i_other_sector)

  res <- c(i_sector, i_other_sector)
  names(res) <- c("i_sector","i_other_sector")
  
  return(res)
}

iterate_sectors <- function(data) {
  res1 <- map_df(sectors, function(x) get_wa_interest_rates_mkt(data,x) )
  res1 <- res1 %>% mutate(sector = sectors) %>% 
    select(sector, everything())
  return(res1)
}

#iterate_sectors(temp)

# res1 <- res1 %>% mutate(sector = sectors) %>% 
#   select(sector, everything())

table_i_iv <- data_r_all %>% 
  group_by(institution_code,period, firm_size) %>% 
  group_modify(~ {.x %>% iterate_sectors()} )
  


#map_df(sectors, function(x) get_wa_interest_rates_mkt(temp,x) )
  
  
#table_i_iv <- expand.grid(sectors, banks, periods,
#              stringsAsFactors=FALSE) 



# 
# data_r_all %>% filter(period == "2016-06-01", 
#                       institution_code == "040002") %>% 
#   group_by(state_code) %>% 
#   summarise(i_other_mkt = mean(interest_rate_disposition))
# 
# 
# data_r_all %>% group_by(period, scian_code) %>% 
#   mutate(other_mkt = ifelse(institution_code != 0, 
#                                       PART_MUN_ENT_ANIO, PART_2 )) %>% 
#   mutate(IAEM_2 = sum(IAEM[AGRUPA == 1], na.rm = TRUE)) %>% 
#   
#   summarise(interest_rate_disposition_other_mkt = 
#                 interest_rate_disposition[institution_code != ])

data_panel <- data_r_all %>% # data set
  group_by(period, institution_code, # grouping vars
           scian_code, firm_size) %>% 
  summarise(N = n(), # aggregated data
              ...
              wa_interest_rate_iv_d = 
                sum(interest_rate_disposition * loan_disposition_month, na.rm = TRUE) / sum(loan_disposition_month, na.rm = TRUE),
...
# we add interest rate from other markets as IV 
data_panel2 <- data_panel %>%
  left_join(table_i_iv,
            by = c("period" = "period",
                   "institution_code" = "institution_code",
                   "scian_code" = "sector", 
                   "firm_size" = "firm_size"))
...
data_period_panel <- data_panel %>% group_by(period) %>% 
    summarise(N = n())

data_period_panel <- data_period %>% 
  left_join(data_period_panel, by = c("period" = "period"))
...
# dummies at industry-year 2023-07-04
data_panel$industry_year <- paste(data_panel$scian_code, data_panel$year, sep = "_")
dum_ind_year <- clase_mat(data_panel,"industry_year")
colnames(dum_ind_year) <- paste("X",colnames(dum_ind_year), sep = "")
data_panel <- cbind(data_panel, dum_ind_year)
industry_year_names <- colnames(dum_ind_year)[order(colnames(dum_ind_year))]
...
pdata$log_mean_disposition <- log(pdata$mean_disposition)
#pdata$wa_term_d <- asinh(pdata$wa_term_d)

form1 <- log_mean_disposition ~ wa_interest_rate_d + wa_term_d + inflation +
  Citibanamex+Santander+HSBC+Inbursa+Scotiabank+Banorte +
  A2016+A2017+A2018+A2019+A2020 #| . + i_other_sector
model_1 <- plm(form1,data = pdata, weights = N_sect, model = "within")
#model_1 <- plm(form1,data = pdata,  model = "within")
summary(model_1)

coef(model_1)["wa_interest_rate_d"] * mean(pdata$wa_interest_rate_d)


form2 <- log_mean_disposition ~ wa_interest_rate_d + wa_term_d + inflation +
  Citibanamex+Santander+HSBC+Inbursa+Scotiabank+Banorte +
  A2016+A2017+A2018+A2019+A2020 + industry_year #| . + i_other_sector
model_2 <- plm(form2,data = pdata, weights = N_sect, model = "within")
#model_2 <- plm(form2,data = pdata, model = "within")
summary(model_2)

coef(model_2)["wa_interest_rate_d"] * mean(pdata$wa_interest_rate_d)

# Export panel  ---------------------------

# export panel 
write.csv(data_panel, paste(path,"/proc/panel_firms_",
                                       which_db,firm_size,".csv", sep = ""), 
          row.names = FALSE)
