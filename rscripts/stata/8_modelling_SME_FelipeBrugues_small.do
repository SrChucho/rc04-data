...
* Importing data
import delimited "$Path/Data/proc/panel_firms_G7small.csv"
...
* NO IV + industry_year 
quietly xtivreg2 log_mean_disposition wa_interest_rate_d wa_term_d inflation citibanamex santander hsbc inbursa scotiabank banorte a2016 a2017 a2018 a2019 a2020 x11_2016-x93_2021 [aweight = n_sect], cluster(scian_code) fe robust
...
* elasticity between disposition and term
est restore no_iv_industry_year
nlcom (_b[wa_term_d] * (mymean_term) ), post 
scalar e_term = _b[_nl_1]
scalar e_term_SE = _se[_nl_1]
...
* IV + industry year
quietly xtivreg2 log_mean_disposition wa_term_d inflation citibanamex santander hsbc inbursa scotiabank banorte a2016 a2017 a2018 a2019 a2020 x11_2016-x93_2021 (wa_interest_rate_d = i_other_sector) [aweight = n_sect], cluster(scian_code) fe robust first
...
quietly xtivreg2 log_mean_disposition log_wa_term_d inflation citibanamex santander hsbc inbursa scotiabank banorte a2016 a2017 a2018 a2019 a2020 x11_2016-x93_2021 (log_wa_interest_rate_d = log_i_other_sector) [aweight = n_sect], cluster(scian_code) fe robust first
...
esttab no_iv no_iv_industry_year iv_model iv_industry_year_log using"$Path/scripts/stata_files/results/small.tex", replace indicate("Industry effects = a2016" "Year effects = a2020" "Industry-Year effects = a2019") nonotes substitute(\_ _) nogaps nogaps compress b(a3) se(a3) star(* 0.10 ** 0.05 *** 0.01) eqlabel(none) nodep label keep(wa_interest_rate_d log_wa_interest_rate_d wa_term_d log_wa_term_d inflation citibanamex santander hsbc inbursa scotiabank banorte) coeflabel(wa_interest_rate_d "Interest rate" log_wa_interest_rate_d "log(Interest rate)" wa_term_d "Term" log_wa_term_d "log(Term)"  inflation "Inflation rate" citibanamex "Citibanamex" santander "Santander" hsbc "HSBC" inbursa "Inbursa" scotiabank "Scotiabank" banorte "Banorte") mtitle("Log Loan Amount") stats(N r2 elas_int elas_int_SE elas_term elas_term_SE weak_iv_test1 weak_iv_test2, labels(Observations R-squared "elas_i" "elas_i_SE" "elas_t" "elas_t_SE" "F first stage" "F first stage"))

********************
* IV + industry_year + log + tcr
quietly xtivreg2 log_mean_disposition log_wa_term_d inflation tcr citibanamex santander hsbc inbursa scotiabank banorte a2016 a2017 a2018 a2019 a2020 x11_2016-x93_2021 (log_wa_interest_rate_d = log_i_other_sector) [aweight = n_sect], cluster(scian_code) fe robust first

estimates store iv_industry_year_log_tcr

estimates restore iv_industry_year_log_tcr
estadd local weak_iv_test2 =  `e(rkf)': iv_industry_year_log_tcr

******************************************	
* write table to file	
******************************************

esttab no_iv no_iv_industry_year iv_model iv_industry_year_log iv_industry_year_log_tcr using"$Path/scripts/stata_files/results/small_tcr.tex", replace indicate("Industry effects = a2016" "Year effects = a2020" "Industry-Year effects = a2019") nonotes substitute(\_ _) nogaps nogaps compress b(a3) se(a3) star(* 0.10 ** 0.05 *** 0.01) eqlabel(none) nodep label keep(wa_interest_rate_d log_wa_interest_rate_d wa_term_d log_wa_term_d inflation tcr citibanamex santander hsbc inbursa scotiabank banorte) coeflabel(wa_interest_rate_d "Interest rate" log_wa_interest_rate_d "log(Interest rate)" wa_term_d "Term" log_wa_term_d "log(Term)"  inflation "Inflation rate" tcr "Real Exchange Rate" citibanamex "Citibanamex" santander "Santander" hsbc "HSBC" inbursa "Inbursa" scotiabank "Scotiabank" banorte "Banorte") mtitle("Log Loan Amount") stats(N r2 elas_int elas_int_SE elas_term elas_term_SE weak_iv_test1 weak_iv_test2, labels(Observations R-squared "elas_i" "elas_i_SE" "elas_t" "elas_t_SE" "F first stage" "F first stage"))

******************************************
* final table to report 

******************************************

* NO IV + industry_year  + log log
quietly xtivreg2 log_mean_disposition log_wa_interest_rate_d log_wa_term_d inflation citibanamex santander hsbc inbursa scotiabank banorte a2016 a2017 a2018 a2019 a2020 x11_2016-x93_2021 [aweight = n_sect], cluster(scian_code) fe robust
estimates store no_iv_industry_year_log

* elasticity between disposition and interest rate
estimates restore no_iv_industry_year_log
nlcom (_b[log_wa_interest_rate_d] * (mymean_i) ), post 
scalar e_int = _b[_nl_1]
scalar e_int_SE = _se[_nl_1]
qui estadd scalar elas_int = e_int: no_iv_industry_year_log
qui estadd scalar elas_int_SE = e_int_SE: no_iv_industry_year_log
* elasticity between disposition and term
est restore no_iv_industry_year_log
nlcom (_b[log_wa_term_d] * (mymean_term) ), post 
scalar e_term = _b[_nl_1]
scalar e_term_SE = _se[_nl_1]
qui estadd scalar elas_term = e_term: no_iv_industry_year_log
qui estadd scalar elas_term_SE = e_term_SE: no_iv_industry_year_log
...
esttab no_iv_industry_year_log iv_industry_year_log using"$Path/scripts/stata_files/results/small_final.tex", replace indicate("Industry effects = a2016" "Year effects = a2020" "Industry-Year effects = a2019") nonotes substitute(\_ _) nogaps nogaps compress b(a3) se(a3) star(* 0.10 ** 0.05 *** 0.01) eqlabel(none) nodep label keep(log_wa_interest_rate_d log_wa_term_d inflation citibanamex santander hsbc inbursa scotiabank banorte) coeflabel(log_wa_interest_rate_d "log(Interest rate)" log_wa_term_d "log(Term)"  inflation "Inflation rate" citibanamex "Citibanamex" santander "Santander" hsbc "HSBC" inbursa "Inbursa" scotiabank "Scotiabank" banorte "Banorte") mtitle("Log Loan Amount") stats(N r2 elas_int elas_int_SE elas_term elas_term_SE weak_iv_test1 weak_iv_test2, labels(Observations R-squared "elas_i" "elas_i_SE" "elas_t" "elas_t_SE" "F first stage" "F first stage"))
